	...
	export default {
		...
		computed:{
        	...
        	sortedandSearchedFics(){
        		...
        		return this.sortedFics.filter(fic => fic.name.toLowerCase().includes(this.searchQuery.toLowerCase()));

        	},
        	filteredandSearchedFics(){

        		return this.$store.state.filteredByGenre.filter(fic => fic.name.toLowerCase().includes(this.searchQuery.toLowerCase()));

        	},
			filteredByCharandSearchedFics(){

				return this.$store.state.filteredByCharacter.filter(fic => fic.name.toLowerCase().includes(this.searchQuery.toLowerCase()));
        ...
        methods:{  
        	...
        	filterByGenre(genre){
        		...
        		console.log(this.$store.state.filteredByGenre)
                ...
                for (let i = 0; i < this.sortedFics.length; i++){
                    for (let j = 0; j < this.sortedFics[i].characters.length; j++){
                        if (character.name == this.sortedFics[i].characters[j].name){
                            ...
                            console.log(this.sortedFics[i].characters[j].name)
                        }
                    }
                }
...
.lunaLibraryPage li.book-like>button:hover{
	...
	.twilightLibraryPage .wordCount, .twilightLibraryPage .author, .twilightLibraryPage .author a, .twilightLibraryPage .preloader span{
		color: $twilightThemeColor2;
	} 
	.celestiaLibraryPage .wordCount, .celestiaLibraryPage .author, .celestiaLibraryPage .author a, .celestiaLibraryPage .preloader span{
		color: $celestiaThemeColor4;
	} 
	.lunaLibraryPage .wordCount, .lunaLibraryPage .author, .lunaLibraryPage .author a, .lunaLibraryPage .preloader span{
